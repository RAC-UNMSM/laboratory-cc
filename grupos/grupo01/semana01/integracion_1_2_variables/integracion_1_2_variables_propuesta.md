...
validacion.py    rol "Validación de entradas"                                   Responsables: Jan - Sebas
matematica.py    rol "Lógica matemática"                                        Responsable: Saúl
visualizacion.py rol "Visualización"                                            Responsable: Cielo
storage.py       rol "Storage/infra"                                            Responsable: Sachy
```
...
1. El usuario llama a la tool `integra_doble_rectangular(expresion, x_min, x_max, y_min, y_max)`. Nótese que se solicitan 5 parámetros: `expresion` es la función de dos variables a integrar y el dominio rectangular queda definido por `x_min`, `x_max`, `y_min` y `y_max`.
...
3. `matematica.py` integra `expresion` simbólicamente usando (`sp.integrate`) según la sintaxis `sp.integrate(expresion, (x, x_min, x_max), (y, y_min, y_max))` habiendo definido `x, y = sp.symbols('x y')`. Se entiende que esta cargada la librería `sympy` como `sp`.

4. `visualizacion.py` grafica la función de dos variables definida por `expresion` (la cual es por definición una superficie en ℝ³) en un archivo PNG (en memoria, nunca en disco — el contenedor no persiste nada entre llamadas).
